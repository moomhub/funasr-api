"""ONNX-backed offline ASR service."""
...
class ONNXOfflineAsrService(RuntimeServiceBase):
    name = "offline_asr_onnx"
    mode = "offline"
    backend = "onnx"

    def preload(self) -> ModelPreloadStatus:
        return self._preload_with(lambda: self.manager.get_offline_onnx_recognizer().load_model())
    ...
    async def recognize(self, request: OfflineAsrRequest) -> RecognitionResult:
        ...
        result = await self.manager.get_offline_onnx_recognizer().recognize(
