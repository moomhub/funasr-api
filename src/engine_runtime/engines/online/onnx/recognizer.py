"""ONNX-backed online recognizer implementation."""
...
from src.engine_runtime.engines.online.onnx.session import OnlineOnnxRealtimeSession


class ONNXOnlineRecognizer(BaseOnlineRecognizer):
    backend_name = "onnx"

    def load_models(self) -> OnlineONNXModelBundle:
        return self.model_manager.get_online_onnx_model_bundle()

    def create_session(self, request: OnlineSessionRequest) -> OnlineOnnxRealtimeSession:
        processing_config = self.model_manager.processing_config
        models = self.load_models()
        return OnlineOnnxRealtimeSession(
    ...
    async def run_inference(self, models: OnlineONNXModelBundle, request: OnlineRecognitionRequest) -> Any:
