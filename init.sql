...
CREATE TABLE IF NOT EXISTS `offline_tasks` (
  `id` VARCHAR(36) NOT NULL COMMENT 'OFFLINE 任务 ID/任务 key',
  ...
  `s3_key` VARCHAR(512) NULL COMMENT '归档后的 S3 key；本地兜底时保存等价相对 key',
  ...
  `is_deleted` BOOLEAN NOT NULL DEFAULT FALSE COMMENT '是否软删除',
  `full_text` TEXT NULL COMMENT '识别后的完整文本',
  `segments` JSON NULL COMMENT '句子/分段级识别结果 JSON',
  `word_timestamps` JSON NULL COMMENT '单字或词级时间戳 JSON，如 [["你",0,200],...]',
...
CREATE TABLE IF NOT EXISTS `spk_tasks` (
  `id` VARCHAR(36) NOT NULL COMMENT 'SPK 任务 ID/任务 key',
  `filename` VARCHAR(255) NOT NULL COMMENT '原始上传文件名',
  `file_size` INT NULL COMMENT '上传文件大小，单位 byte',
  `email` VARCHAR(255) NULL COMMENT '用户邮箱；为空时不发送完成消息',
  `vip` BOOLEAN NOT NULL DEFAULT FALSE COMMENT '是否 VIP 优先任务',
...
CREATE TABLE IF NOT EXISTS `s3_files` (
  ...
  `bucket_name` VARCHAR(255) NULL COMMENT 'S3 bucket 名称；本地兜底时为空',
  `s3_key` VARCHAR(512) NOT NULL COMMENT 'S3 对象 key；本地兜底时保存等价相对 key',
  ...
  `local_path` VARCHAR(1024) NULL COMMENT '本地兜底文件绝对或相对路径；S3 成功时为空',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='S3/本地兜底文件索引表';
